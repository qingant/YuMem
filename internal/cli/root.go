...
func printStartupBanner(config StartupConfig) {
	...
	// MCP API endpoints
	fmt.Println("🔗 MCP Protocol (Streamable HTTP):")
	fmt.Printf("   ├─ Endpoint       : http://localhost:%d/mcp\n", config.MCPPort)
	fmt.Println("   ├─ High-level     : store_memory, recall_memory, get_core_memory")
	fmt.Println("   ├─ Low-level      : get_l0_context, update_l0, consolidate_l0,")
